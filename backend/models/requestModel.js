/**
 * models/requestModel.js -- everything that touches `requests`.
 *
 * accept/reject run here, not in the controller, because a decision must
 * touch THIS request, the OTHER pending requests, and the item's status
 * in one transaction. Split across layers, you get an Accepted request
 * whose item is still Available.
 */
...
/**
 * Revives a REJECTED request from the same pair back to Pending.
 *
 * The `status: 'Rejected'` guard is the safety belt: this can only revive
 * a dead request, never overwrite a live Pending or Accepted one. If the
 * row stopped being Rejected between read and write, zero rows change and
 * we return null -- which the controller reports as a 409.
 */
 ...
 * pending request for the same item Rejected, the item Reserved. `ownerId`
 * MUST be req.user.id; re-checked against the locked row so a race cannot
 * accept on someone else's behalf. Returns { ok, data } or { ok, reason }.
 */
